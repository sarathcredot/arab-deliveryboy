...
const GET__WARRANTY = gql`
 query GetPendingWarrantyPickupsByAgent($input: getPendingWarrantyPickupsByAgentInput) {
  getPendingWarrantyPickupsByAgent(input: $input) {
    success
    data {
      user
      product
...
const Warranties = () => {
  ...
  document.title = "Orders | Arabdeals-Agent & Dashboard";
   ...
   const [currentPage, setCurrentPage] = useState(null);
    ...
    useEffect(() => {
      if (ordersData && ordersData.getPendingWarrantyPickupsByAgent) {
        console.log("response",ordersData)
        setOrders(ordersData.getPendingWarrantyPickupsByAgent.data);
      }
    }, [ordersData]);
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid className="px-2">
          {/* Render Breadcrumbs */}
          <Breadcrumbs
            title="Pending Warranty Pickups"
            onFilterChange={handleFilterChange}
            isHistory={true}
          />
          <div className="order-outer-div">
         {ordersData?.getPendingWarrantyPickupsByAgent?.data?.length === 0 ? (
